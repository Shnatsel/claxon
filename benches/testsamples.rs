...
fn bench_decode(path: &str, c: &mut Criterion) {
    ...
    c.bench_function(path, move |bench| bench.iter(|| {
        let mut should_refresh = true;
        {
            ...
            if let Some(b) = block {
                bytes += b.len() * (bps / 8);
                iterations += 1;
                buffer = criterion::black_box(b.into_buffer());
...
fn bench_p0_mono_16bit(bencher: &mut Criterion) {
    bench_decode("testsamples/p0.flac", bencher);
}

fn bench_p1_stereo_24bit(bencher: &mut Criterion) {
    bench_decode("testsamples/p1.flac", bencher);
}

fn bench_p2_stereo_16bit(bencher: &mut Criterion) {
    bench_decode("testsamples/p2.flac", bencher);
}

fn bench_p3_stereo_16bit(bencher: &mut Criterion) {
    bench_decode("testsamples/p3.flac", bencher);
}

fn bench_p4_stereo_16bit(bencher: &mut Criterion) {
...
criterion_group!{
    ...
    targets = bench_p0_mono_16bit, bench_p1_stereo_24bit, bench_p2_stereo_16bit, bench_p3_stereo_16bit, bench_p4_stereo_16bit
}
